...
- **Cortina de acceso REAL** (`src/middleware.ts` + `/entrar`): contraseña en `SITE_GATE_PASSWORD`, solo en servidor, cookie `HttpOnly`. Para abrir la web al público **basta con borrar esa variable en Vercel**.
...
- **Una diapositiva por texto también en escritorio**, en `.02` y `.03` de la home. Se reutiliza la maquinaria del móvil (`.rv-panel` + `prepararBarrido`), con opción explícita por bloque vía `data-slides`. En escritorio son **dos paradas, no cuatro**: izquierda, derecha, y vuelta a empezar, emparejadas con la dirección del barrido. Medida de lectura de 62ch en cuerpo y 25ch en titulares. La llamada a la acción **ya no se lleva una pantalla para ella sola**, viaja con su texto (el móvil baja de 10 a 8 tramos).
...
- **Brandbook · Identidad visual — una diapositiva por texto también en escritorio (2026-08-06).** "Un scroll, una animación, una diapositiva" pasa de ser comportamiento solo de móvil a patrón de la marca, activable por bloque. En escritorio son **dos paradas** (izquierda, derecha) frente a las cuatro del móvil, emparejadas con la dirección del barrido, con medida de lectura acotada (62ch cuerpo, 25ch titulares).
